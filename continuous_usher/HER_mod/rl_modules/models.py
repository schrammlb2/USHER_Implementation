...
class T_conditioned_ratio_critic(nn.Module):
    ...
    def forward(self, x, T, actions, return_p=False):
        ...
        if return_p: 
            ...
            lt_zero = (val <= 0)*1
            # val.clamp(-100, 100)
            # #do a weird mapping here to prevent p from blowing up too much
            # p_value =  torch.clamp(2**(val), 0.0001, 1)*lt_zero + (val + 1)*(1-lt_zero)

            #do a weird mapping here to prevent p from blowing up too much
            # p_value =  torch.clamp(2**(val), 0.0001, 1)*lt_zero + (torch.abs(val + 1)**.5)*(1-lt_zero)
            # p_value =  torch.clamp(2**(val), 0.0001, 1)*lt_zero + (val + 1)*(1-lt_zero)
            # p_value = torch.where(lt_zero, 2**val, val)
            p_value = val
            # p_value = torch.where(p_value > 0, p_value**.5, p_value)
            p_value = torch.nn.ELU()(p_value) + 1
            # p_value.clamp(0.01, 1000000)
            # p_value.clamp(0.01, 10e8)
            # p_value = p_value.clamp(0.0001, 10e6)
            # p_value = p_value**.5

            p_value.clamp(0.001, 100000)
            assert (p_value == p_value).all()
            assert (q_value == q_value).all()

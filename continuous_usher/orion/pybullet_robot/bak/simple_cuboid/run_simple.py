...
car = p.loadURDF("simple_cuboid.urdf",[0,0,0], [0,0,0,1])
...
if not seperate_wheel:
    ...
    maxForceSlider = p.addUserDebugParameter("maxForce", 0, 100, 0)
else:
    for key, wheel in wheelIds_dict.items():
        wheel["vecId"] = p.addUserDebugParameter(key + "wheelVelocity", -200, 200, 0)
        wheel["forceId"] = p.addUserDebugParameter(key + "maxForce", 0, 100, 0)
...
while (1):
    ...
    for key, wheel in wheelIds_dict.items():
        ...
        p.setJointMotorControl2(bodyUniqueId=car, jointIndex=wheel["id"], 
                                controlMode=p.VELOCITY_CONTROL, targetVelocity = targetVelocity, 
                                force = maxForce)
        if GUI_friction:
    ...
    # robotPos, robotOri = p.getBasePositionAndOrientation(car)

...
@ti.layout
def place():
  ti.root.dense(ti.l, max_steps).dense(ti.i, n_objects).place(x, x_saved, v, v_saved, rotation,
                                                              omega, v_inc,
                                                              omega_inc)
...
def compute_loss(t: ti.i32):
    for i in range(0,steps):
        ti.atomic_add(loss[None],0.5*(x_saved[i,0][0]-x[i,0][0])**2 + 0.5*(x_saved[i,0][1]-x[i,0][1])**2 + 0.5*(v_saved[i,0][0]-v[i,0][0])**2 + 0.5*(v_saved[i,0][1]-v[i,0][1])**2)
...
def forward(output=None, visualize=True):
  ...
  #f_state.write('%f %f %f %f\n'%(x[0,0][0],x[0,0][1],v[0,0][0],v[0,0][1]))

  for t in range(1, total_steps):
    collide(t - 1)
    advance(t)

    #f_state.write('%f %f %f %f\n'%(x[t,0][0],x[t,0][1],v[t,0][0],v[t,0][1]))
...
def main():
    ...
    with open('state.txt','r') as f_state:
        for idx,line in enumerate(f_state.readlines()):
            line = line.rstrip('\n')
            s_list=[float(i) for i in line.split(' ')]
            x_saved[idx,0] = [s_list[0],s_list[1]]
            v_saved[idx,0] = [s_list[2],s_list[3]]
    ...
    for i in range(25):
        ...
        friction[None] -= learning_rate * friction.grad[None]
        losses.append(loss[None])
        values.append(friction[None])
        print('Iter=',i,', Loss=',loss[None],', Friction: ',friction[None])

    plt.plot(values,losses,'go--',linewidth=2,label='Loss')
    plt.show()

...
def format_title(inpt): 	
	...
	return inpt#(inpt.replace("RandomGridworld", " Random Obstacles")).replace("Gridworld", " Continuous Long/Short Path Environment")
...
def line_plot(experiment_dict, name="Environment"):
	...
	for key in experiment_dict[methods[0]].keys():
		...
		for method in experiment_dict.keys():
			...
			method_name = format_method(method)
			# pdb.set_trace()
			if not ("bellman" in method_name):
				color = color_map(method_name)
				plt.plot(noise_list, mean_list, color=color,label=method_name)
				# plt.fill_between(noise_list, lower_ci_list, upper_ci_list, color=color_list[i], alpha=.1)
				plt.fill_between(noise_list, lower_ci_list, upper_ci_list, color=color, alpha=.1)
